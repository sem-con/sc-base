    ...
    def getData(params)
        if ENV["WATERMARK"].to_s == ""
            if params.to_s.starts_with?("id=")
                retVal = [Store.select(:id, :item, :dri, :schema_dri, :table_name, :created_at, :updated_at, :mime_type).find(params[3..-1])].map(&:serializable_hash) rescue []
            elsif params.to_s.starts_with?("dri=")
                retVal = [Store.select(:id, :item, :dri, :schema_dri, :table_name, :created_at, :updated_at, :mime_type).find_by_dri(params[4..-1])].map(&:serializable_hash) rescue []
            elsif params.to_s.starts_with?("schema_dri=")
                retVal = Store.select(:id, :item, :dri, :schema_dri, :table_name, :created_at, :updated_at, :mime_type).where(dri: params[11..-1]).to_a.map(&:serializable_hash) rescue []
            elsif params.to_s.starts_with?("table_name=")
                if params[11..-1].to_s == "default"
                    retVal = Store.select(:id, :item, :dri, :schema_dri, :table_name, :created_at, :updated_at, :mime_type).where(table_name: [nil, "", "default"]).to_a.map(&:serializable_hash) rescue []
                else
                    retVal = Store.select(:id, :item, :dri, :schema_dri, :table_name, :created_at, :updated_at, :mime_type).where(table_name: params[11..-1]).to_a.map(&:serializable_hash) rescue []
                end
            else
                retVal = Store.select(:id, :item, :dri, :schema_dri, :table_name, :created_at, :updated_at, :mime_type).to_a.map(&:serializable_hash)
            end
        else
            retVal = []
